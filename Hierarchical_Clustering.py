...
# Visualize Dendrogram to find optional number of clusters
dendrogram = sch.dendrogram(sch.linkage(X, method='ward'), truncate_mode='lastp',
                            p=12, leaf_rotation=45., leaf_font_size=15., show_contracted=True)
...
plt.scatter(X[y_ac == 3, 0], X[y_ac == 3, 1],
            s=100, c='black', label='Careless')
plt.scatter(X[y_ac == 4, 0], X[y_ac == 4, 1],
            s=100, c='grey', label='Sensible')
